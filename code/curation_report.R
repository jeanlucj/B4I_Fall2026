...
# collapsed and on what grounds: CURATION.md at the repository root.
#
...
#
# Outputs: CURATION.md (repository root, beside the other .md files, so it is
#          versioned rather than regenerated -- it is meant to be read and
#          linked to, not just produced)
# ============================================================
...
# The curation outputs it reads live in output/; the report it writes lives at
# the root with the other documentation.
out_dir  <- here::here("output")
out_file <- here::here("CURATION.md")
...
species_block <- function(sp, label) {
  ...
  if (!is.null(groups) && nrow(groups) > 0) {
    ...
    out <- paste0(out, "\n### Near-identical groups\n\n",
      ...
      "Full membership in `output/", sp, "_identity_groups.csv`.\n")
  }
...
header <- paste0(
  "# CURATION\n\n",
  "What was collapsed, and on what grounds. For usage see ",
  "[README.md](README.md); for structure see [DESIGN.md](DESIGN.md); for why ",
  "the thresholds are what they are see [BACKGROUND.md](BACKGROUND.md).\n\n",
  ...
  "files in `output/`, not whatever a threshold has since been changed to. ",
  "Re-run a species' curation and then this script after changing one.\n\n",
  ...
  "`output/oat_analysis_names.csv` and `output/pea_analysis_names.csv`; ",
  "the reasoning behind the thresholds is in [BACKGROUND.md](BACKGROUND.md).\n\n",
  "Naming convention:\n\n",
  "| analysis name | means |\n|---|---|\n",
